...
class iotPin {
	public:
		...
		static inline uint8_t	getPinMask(iotPinId pin) {
			return 1 << (pin % iotcNumPinsPerPorts);
		}
...
class iotClock {
	public:
		...
		processOne() {
			...
			if((days = iSeconds / 86400L) != iYearDays) {
				...
				if(iCharLcd) iCharLcd->print(iRow, 1, iMonDyHrMnSc);
			}
			toTime(iMonDyHrMnSc + 7, iSeconds);
			if(iCharLcd) iCharLcd->print(iRow, 8, iMonDyHrMnSc + 7);
			executeSchedules();
		}
		static void
	...
	private:
		...
		update() {
			iSeconds++;
			if(iSeconds >= 31104000L) iSeconds = 0;
			iFlush = 1;
		}
